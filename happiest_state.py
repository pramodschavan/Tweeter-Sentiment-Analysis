...
def parse_tweet(tweetlist,sentdic):
    ...
    output_file = open('output_happystates.txt','w')
	...
	if "place" in tweet.keys() and tweet['place'] is not None and tweet['place']['country_code'] == 'US':
		...
		if "text" in tweet.keys() and tweet['lang'] == "en":   
			...
			''' Appending sentiment scores for non-sentiment words in tweets '''				
			for word in accum_dic.keys():
				#print word + "\n"
				accum_dic[word].append(score)
			
			''' Updating happiness dictionary to capture happiness of each US state '''
			if location not in happiness_dic.keys():
				happiness_dic[location] = score
			else:
				happiness_dic[location] = happiness_dic[location] + score
			
    ''' Reading of all tweets done '''			
    
    ''' printing happiness dictionary in sorted order'''
    for state in sorted(happiness_dic, key = happiness_dic.get, reverse=True):
    	print state + ' ' + str(happiness_dic[state])
    	output_file.write(state + ' ' + str(happiness_dic[state])+ '\n')
